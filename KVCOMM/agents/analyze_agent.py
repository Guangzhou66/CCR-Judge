...
def build_diverse_wiki_queries(question, entities, agent_id):
    """Build slightly different wiki queries to encourage answer diversity.

    - `entities` are the raw strings extracted between '@...@' from the
      Knowledgeable Expert.
    - We derive different patterns based on the current node id so that
      multiple Wiki Searcher agents use different query styles.
    """
    clean_entities = [
    ...
    short_question = (question or "").split("\n", 1)[0].strip()
    try:
        idx = int(agent_id) if agent_id is not None else 0
    except Exception:
        idx = 0
    pattern = idx % 3

    if pattern == 0:
        queries = clean_entities
    elif pattern == 1:
        queries = [f"{name} definition" for name in clean_entities]
    else:
        queries = [
            f"{name} in the context of {short_question}"
    ...
    return unique_queries or [short_question or clean_entities[0]]
                ...
                for agent_id, info in spatial_info.items():
                    if self.role == 'Wiki Searcher' and info['role'] == 'Knowledgeable Expert':
                        ...
                        if raw_inputs['task'] in source_memory.get('condition', {}):
                            continue
                        raw_queries = find_strings_between_pluses(info['output'])
                        queries = build_diverse_wiki_queries(raw_inputs['task'], raw_queries, self.id)
                        try:
