    ...
    async def _process_inputs(
        ...
        **kwargs,
        ...
        if mode == "allow_kv_reuse":
            ...
            fewshot_ids = [aid for aid, info in spatial_info.items() if info.get("role") == "FewShotRetriever"]
            other_ids = [aid for aid, info in spatial_info.items() if info.get("role") != "FewShotRetriever"]

            try:
                self_index = int(self.id)
            except Exception:
                self_index = 0
            if fewshot_ids:
                offset = self_index % len(fewshot_ids)
                fewshot_ids = fewshot_ids[offset:] + fewshot_ids[:offset]

            ordered_ids = other_ids + fewshot_ids

            for agent_id in ordered_ids:
                info = spatial_info[agent_id]
                role = info["role"]
                if role == "FewShotRetriever":
                    agent_output = "A relevant solved example is:\n{condition_" + agent_id + "_current}"
                else:
                ...
                spatial_str += (
                    f"Agent {agent_id} as a {role} his answer to this question is:\n\n"
                    f"{agent_output}\n\n"
                )
