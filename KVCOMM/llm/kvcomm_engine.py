...
class _RequestState:
    ...
    def __init__(
        ...
        # Debug/analysis: per-request attribution for judge-group-agents.
        # Shape: target_placeholder -> {"weight_by_source": {source_placeholder: float}, "count_by_source": {...}}
        self.judge_group_anchor_usage: Dict[str, Any] = {}
...
class KVCOMMEngine:
    ...
    def offset_kv_cache_pair(
        ...
        # Ensure all selected anchors are compatible in prefix length.
        base_prefix_key, base_prefix_value = self._stack_cache_tensors(base_prefix_cache)
        target_pf_len = base_prefix_key.shape[-2]

        # Filter anchors whose prefix delta is too short and crop longer ones.
        original_anchor_index = anchor_index
        filtered_indices: List[int] = []
        filtered_positions: List[int] = []
        for pos, idx in enumerate(original_anchor_index):
            pf_delta = anchor_list[idx].get(f"{self.llm.node_id}_pf_key_delta")
            if pf_delta is None:
                continue
            if pf_delta.shape[-2] >= target_pf_len:
                filtered_indices.append(idx)
                filtered_positions.append(pos)

        anchor_index = filtered_indices
        if not anchor_index:
            # No compatible anchors for this placeholder/prefix span.
            return base_placeholder_cache.copy(), base_prefix_cache.copy()

        # Slice all weight tensors to keep only the filtered anchors.
        weights_key_for_prefix = weights_key_for_prefix[filtered_positions]
        weights_value_for_prefix = weights_value_for_prefix[filtered_positions]
        weights_key_for_placeholder = weights_key_for_placeholder[filtered_positions]
        weights_value_for_placeholder = weights_value_for_placeholder[filtered_positions]

        # If judge-group-agents is enabled, attribute which source agent anchors contribute
        # to each target agent placeholder segment.
        try:
            if (
                getattr(self.llm.config, "group_judge_agent_anchors", False)
                and getattr(self.llm, "role", None) == "FinalSelectBest"
                ...
                if usage is None:
                    state.judge_group_anchor_usage = {}
                    usage = state.judge_group_anchor_usage
                target_entry = usage.setdefault(
                    ph_id,
                    {"weight_by_source": {}, "count_by_source": {}},
                )
                weight_by_source: Dict[str, float] = target_entry.setdefault("weight_by_source", {})
                count_by_source: Dict[str, int] = target_entry.setdefault("count_by_source", {})
                # weights_* tensors have leading dim aligned with current `anchor_index`.
                for pos, anchor_idx in enumerate(anchor_index):
                    anchor = anchor_list[anchor_idx]
                    if not isinstance(anchor, dict):
                        continue
                    source = str(anchor.get("_source_ph_id") or "unknown")
                    w = float(weights_value_for_placeholder[pos].float().mean().item())
                    weight_by_source[source] = weight_by_source.get(source, 0.0) + w
                    count_by_source[source] = count_by_source.get(source, 0) + 1
        except Exception:
            # Never fail generation for analysis logging.
            pass

        prefix_key_delta_stack = torch.stack(
            [
                anchor_list[i][f"{self.llm.node_id}_pf_key_delta"][..., :target_pf_len, :]
        ...
        prefix_value_delta_stack = torch.stack(
            [
                anchor_list[i][f"{self.llm.node_id}_pf_value_delta"][..., :target_pf_len, :]
                for i in anchor_index
            ]
        )
    ...
    def set_anchor(
        ...
        def _make_anchor(i, ph_id, real_ph, base_ph, real_pf, base_pf):
            ...
            # For judge position-grouping, record the display slot for later filtering.
            try:
                if (
                    getattr(self.llm.config, "group_judge_agent_anchors_by_position", False)
                    and getattr(self.llm, "role", None) == "FinalSelectBest"
                ):
                    pos_map = getattr(state, "judge_display_pos_by_ph_id", None) or {}
                    display_pos = pos_map.get(ph_id)
                    if display_pos is not None:
                        entry["_display_pos"] = int(display_pos)
            except Exception:
                pass
            return i, entry
