    ...
    def get_spatial_info(self, message=None, prefix=False)->Dict[str,Dict]:
        ...
        if self.spatial_predecessors is not None:
            for predecessor in self.spatial_predecessors:
                ...
                predecessor_outputs_meta = (
                    getattr(predecessor, "outputs_meta", [])
                    if message is None
                    else (getattr(predecessor, "outputs_meta", {}) or {}).get(message, [])
                )
                ...
                predecessor_meta = {}
                if isinstance(predecessor_outputs_meta, list) and len(predecessor_outputs_meta):
                    maybe_meta = predecessor_outputs_meta[-1]
                    if isinstance(maybe_meta, dict):
                        predecessor_meta = maybe_meta
                spatial_info[predecessor.id] = {
                    "role": predecessor.role,
                    "output": predecessor_output,
                    "metadata": predecessor_meta,
                }
    ...
    async def async_execute(self, input:Any, mode: str = "default", **kwargs):
        ...
        if mode == "default":
            ...
            for result in results:
                ...
                for item in result:
                    ...
                    self.outputs_meta.append(generation.metadata or {})
                    if request_uid:
        ...
        if mode == "allow_kv_reuse":
            ...
            for (message, result) in results:
                ...
                if message not in self.outputs_meta:
                    self.outputs_meta[message] = []
                for item in result:
                    if isinstance(item, GenerationResult):
                        generation = item
                    else:
                    ...
                    self.outputs_meta[message].append(generation.metadata or {})
                    if request_uid:
                        metrics_recorder.record_agent_output(
                            request_uid=request_uid,
                            agent_id=self.id,
